...
def detect_sweep_events(df, swing_window=5, expiry_bars=100, confirmation="close"):
    ...
    for i in range(n):
        ...
        # 2) KEYIN: shu svecha yangi swing bo'lsa, uni faol daraja qilib belgilaymiz
        if i in swing_high_set:
            active_idx = i
            active_level = highs[i]
    ...
    for i in range(n):
        if active_level is not None:
            if i - active_idx > expiry_bars:
                active_idx = None
                active_level = None
            elif lows[i] <= active_level:
                kind = "trap" if closes[i] > active_level else "break"
                events.append({
                    "type": kind,
                    "direction": "bullish" if kind == "trap" else "bearish",
                    "level": float(active_level),
                    "level_time": str(times[active_idx]),
                    "event_time": str(times[i]),
                    "event_idx": i,
    ...
    events.sort(key=lambda e: e["event_idx"])
    return events


def get_latest_active_levels(df, swing_window=5, expiry_bars=100):
    """Joriy (oxirgi) vaqtda FAOL (hali sweep qilinmagan, muddati o'tmagan)
    resistance va support darajalarini qaytaradi - agar mavjud bo'lsa."""
    highs = df["high"].values
    lows = df["low"].values
    n = len(df)
    swing_high_idx, swing_low_idx = find_pivots(highs, lows, window=swing_window)
    ...
    for i in swing_high_idx:
        if n - 1 - i <= expiry_bars:
            active_high = {"level": float(highs[i]), "idx": i, "time": str(df.index[i])}

    active_low = None
    for i in swing_low_idx:
        if n - 1 - i <= expiry_bars:
            active_low = {"level": float(lows[i]), "idx": i, "time": str(df.index[i])}

    return {"resistance": active_high, "support": active_low}
